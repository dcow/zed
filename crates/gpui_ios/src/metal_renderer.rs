...
// iOS uses UIKit — no AppKit or CoreVideo dependencies.
use gpui::{
...
pub(crate) struct MetalRenderer {
    ...
    // CVMetalTextureCache is not used on iOS Phase 1 — video surface rendering
    // will be added in a later phase when CoreVideo is linked.
...
impl MetalRenderer {
    ...
    pub fn new(instance_buffer_pool: Arc<Mutex<InstanceBufferPool>>, transparent: bool) -> Self {
        ...
        unsafe {
            ...
            // Note: setAutoresizingMask is not used on iOS since CAMetalLayer
            // sizing is driven by UIView bounds changes.
        }
    ...
    pub fn update_drawable_size(&mut self, size: Size<DevicePixels>) {
        if let Some(layer) = &self.layer {
            // CGSize encoding for msg_send! on iOS/arm64
            let cg_size = crate::display::CGSize {
    ...
    ///
    /// Note: This requires a layer-backed renderer. Not implemented on iOS Phase 1.
    #[cfg(any(test, feature = "test-support"))]
    // render_to_image and render_scene_to_image are not implemented for iOS Phase 1.
    // Visual testing on iOS uses the simulator's live render path.
    ...
    fn draw_surfaces(
        ...
        // Video surface rendering requires CoreVideo YUV texture cache support.
        // This is not yet implemented on iOS Phase 1; surfaces are silently dropped.
        if !surfaces.is_empty() {
            log::warn!(
                "gpui_ios: {} video surface(s) skipped — not yet implemented",
                surfaces.len()
            );
        }
        true
...
// MetalHeadlessRenderer is not implemented for iOS Phase 1.
// Test support on iOS uses the simulator's actual render path.
